...
EH:
  ...
  A.min.sample.frac: x
ensemble:
  # fill in "HipSTR", "GangSTR", "EH", or "no caller"
  # caller.one cannot be filled in with "no caller"
  # caller.two can only be "no caller" if caller.three is also "no caller"
  # caller.three can always be "no caller"
  nonA.caller.one: x
  nonA.caller.two: x
  nonA.caller.three: x
  A.caller.one: x
...
filtering_function <-
  ...
  subset_data <- subset_data %>%
    ...
    mutate(caller = if_else(get(paste0(caller.one,"_passlocus")) == T & caller.one != "no caller", caller.one,
                            if_else(get(paste0(caller.two,"_passlocus")) == T & caller.two != "no caller", caller.two,
                                    if_else(get(paste0(caller.three,"_passlocus")) == T & caller.three != "no caller", caller.three, "no call"))))
