...
// Temporary directory for SortSam
params.sortsam_tmp = '/state/partition1/cal8150/gatk-tmp'
...
process SORT_BAM {
  ...
  java -jar ${params.picard} SortSam \\
    ...
    TMP_DIR=${params.sortsam_tmp}; samtools index ${sampleID}.unmarked.sorted.bam
...
if (sex == "M") {
  ...
  results <- results %>%
    ...
    mutate(H_REPDIFF_1 = H_GB_1/nchar(motif.family),
           H_REPDIFF_2 = if_else((chr == "chrX" | chr == "chrY"), -999, H_GB_2/nchar(motif.family))) %>%
    # calculate repeat count
    # fill in -999 for allele 2 on sex chromosomes to indicate it should not be considered a true call
    mutate(H_REPCN_1 = (width + H_GB_1)/nchar(motif.family),
           H_REPCN_2 = if_else((chr == "chrX" | chr == "chrY"), -999, (width + H_GB_2)/nchar(motif.family)))
  ...
  results <- results %>% 
    ...
    mutate(H_ALLREADS_1 = H_ALLREADS_1_TEMP, H_ALLREADS_2 = H_ALLREADS_2_TEMP,
           H_GB_1 = H_GB_1_TEMP, H_GB_2 = H_GB_2_TEMP) %>%
    mutate(H_REPDIFF_1 = if_else(chr == "chrY", -999, H_GB_1/nchar(motif.family)), 
           H_REPDIFF_2 = if_else(chr == "chrY", -999, H_GB_2/nchar(motif.family))) %>%
    mutate(H_REPCN_1 = if_else(chr == "chrY", -999, (width + H_GB_1)/nchar(motif.family)), 
           H_REPCN_2 = if_else(chr == "chrY", -999, (width + H_GB_2)/nchar(motif.family)))
...
filtering_function <-
  ...
  subset_data <- subset_data %>% mutate(allele_1_repdiff = if_else(caller == caller.one & !!as.name(paste0(caller.one,"_passsample")) == T, !!as.name(paste0(substr(caller.one,1,1),"_REPDIFF_1")), NA_real_))
  # Genotypes for caller.two calls
  if(caller.two != "no_caller") {
    subset_data <- subset_data %>% mutate(allele_1_repdiff = if_else(caller == caller.two & !!as.name(paste0(caller.two,"_passsample")) == T, !!as.name(paste0(substr(caller.two,1,1),"_REPDIFF_1")), allele_1_repdiff))
  }
  # Genotypes for caller.three calls
  if(caller.three != "no_caller") {
    subset_data <- subset_data %>% mutate(allele_1_repdiff = if_else(caller == caller.three & !!as.name(paste0(caller.three,"_passsample")) == T, !!as.name(paste0(substr(caller.three,1,1),"_REPDIFF_1")), allele_1_repdiff))
  }
  
  # Fill in allele_2
  # Genotypes for caller.one calls, and set remaining to NA
  subset_data <- subset_data %>% mutate(allele_2_repdiff = if_else(caller == caller.one & !!as.name(paste0(caller.one,"_passsample")) == T, !!as.name(paste0(substr(caller.one,1,1),"_REPDIFF_2")), NA_real_))
  # Genotypes for caller.two calls
  if(caller.two != "no_caller") {
    subset_data <- subset_data %>% mutate(allele_2_repdiff = if_else(caller == caller.two & !!as.name(paste0(caller.two,"_passsample")) == T, !!as.name(paste0(substr(caller.two,1,1),"_REPDIFF_2")), allele_2_repdiff))
  }
  # Genotypes for caller.three calls
  if(caller.three != "no_caller") {
    subset_data <- subset_data %>% mutate(allele_2_repdiff = if_else(caller == caller.three & !!as.name(paste0(caller.three,"_passsample")) == T, !!as.name(paste0(substr(caller.three,1,1),"_REPDIFF_2")), allele_2_repdiff))
  }
...
## 3. genotype_qc.r (for non-Mendelian data)
...
# non poly-A loci stats
  ...
  nonA_stats <-
    data.frame(
      ...
      nonA_num_2bp_loci_genotyped = length(which(nchar(nonA_results_stats$motif.family) == 2 & !is.na(nonA_results_stats$allele_1))),
      nonA_num_3bp_loci_genotyped = length(which(nchar(nonA_results_stats$motif.family) == 3 & !is.na(nonA_results_stats$allele_1))),
      nonA_num_4bp_loci_genotyped = length(which(nchar(nonA_results_stats$motif.family) == 4 & !is.na(nonA_results_stats$allele_1))),
