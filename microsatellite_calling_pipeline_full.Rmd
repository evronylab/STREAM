...
error_report_function <-
  ...
  subset_data <- subset_data %>%
    ...
    mutate(sample_failed_filters = if_else(H_GT != "." & !is.na(H_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_GT",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_Q >= min.qual.hipstr & !is.na(H_Q),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_Q",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MALLREADS_SUM >= min.mallreads.hipstr & !is.na(H_MALLREADS_SUM),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_MALLREADS_SUM",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_ALLREADS_SUM >= min.allreads.hipstr & !is.na(H_ALLREADS_SUM),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_ALLREADS_SUM",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MALLREADS_1 >= min.allele.mallreads.hipstr & !is.na(H_MALLREADS_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_MALLREADS_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_MALLREADS_2 >= min.allele.mallreads.hipstr & !is.na(H_MALLREADS_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_MALLREADS_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_ALLREADS_1 >= min.allele.allreads.hipstr & !is.na(H_ALLREADS_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_ALLREADS_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_ALLREADS_2 >= min.allele.allreads.hipstr & !is.na(H_ALLREADS_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_ALLREADS_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MVAF_1 >= min.mvaf.hipstr & !is.na(H_MVAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_MVAF_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_MVAF_2 >= min.mvaf.hipstr & !is.na(H_MVAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_MVAF_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_AVAF_1 >= min.avaf.hipstr & !is.na(H_AVAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_AVAF_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_AVAF_2 >= min.avaf.hipstr & !is.na(H_AVAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_AVAF_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_STUTTERFRAC <= max.stutter.frac.hipstr & !is.na(H_STUTTERFRAC),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_STUTTERFRAC",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_FLINDELFRAC <= max.flankindel.frac.hipstr & !is.na(H_FLINDELFRAC),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_FLINDELFRAC",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_GLDIFF >= min.gldiff.hipstr | is.na(H_GLDIFF),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"H_GLDIFF",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_GT != "." & !is.na(G_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_GT",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_Q >= min.qual.gangstr & !is.na(G_Q),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_Q",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_ENCLREADS_SUM >= min.total.reads.gangstr & !is.na(G_ENCLREADS_SUM),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_ENCLREADS_SUM",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_ENCLREADS_1 >= min.allele.reads.gangstr & !is.na(G_ENCLREADS_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_ENCLREADS_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             G_ENCLREADS_2 >= min.allele.reads.gangstr & !is.na(G_ENCLREADS_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_ENCLREADS_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_VAF_1 >= min.vaf.gangstr & !is.na(G_VAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_VAF_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             G_VAF_2 >= min.vaf.gangstr & !is.na(G_VAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"G_VAF_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_GT != "." & !is.na(E_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_GT",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(B_depth >= min.total.reads.eh & !is.na(B_depth),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"B_depth",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_ADSP_1 >= min.allele.reads.eh & !is.na(E_ADSP_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_ADSP_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             E_ADSP_2 >= min.allele.reads.eh & !is.na(E_ADSP_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_ADSP_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_VAF_1 >= min.vaf.eh & E_VAF_1 <= max.vaf.eh & !is.na(E_VAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_VAF_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             E_VAF_2 >= min.vaf.eh & E_VAF_2 <= max.vaf.eh & !is.na(E_VAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_VAF_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_LC >= min.lc.eh & !is.na(E_LC),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"E_LC",sep=";"),"none;"))) %>%
  ...
  # fill with "none" if passed
    ...
    mutate(locus_failed_filters = if_else(mean_qual_H >= min.mean.qual.hipstr & !is.na(mean_qual_H),
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_qual_H",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_qual_G >= min.mean.qual.gangstr & !is.na(mean_qual_G),
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_qual_G",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_mdepth_H >= min.mean.mallreads.hipstr,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_mdepth_H",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_adepth_H >= min.mean.allreads.hipstr,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_adepth_H",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_depth_G >= min.mean.total.reads.gangstr,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_depth_G",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_depth_E >= min.mean.total.reads.eh,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"mean_depth_E",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_H == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"sample_frac_filter_H",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_G == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"sample_frac_filter_G",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_E == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"sample_frac_filter_E",sep=";"),"none;")))
