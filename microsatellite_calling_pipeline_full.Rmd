...
nonA.min.lc.eh <- as.numeric(args[51])
A.min.lc.eh <- as.numeric(args[52])
...
nonA.min.mean.total.reads.eh <- as.numeric(args[53])
A.min.mean.total.reads.eh <- as.numeric(args[54])
# minimum fraction of total samples that have a call that passed all sample-level filters
# nonA.min.sample.frac.eh <- config$EH$nonA.min.sample.frac
# A.min.sample.frac.eh <- config$EH$A.min.sample.frac
nonA.min.sample.frac.eh <- as.numeric(args[55])
A.min.sample.frac.eh <- as.numeric(args[56])
...
nonA.caller.one <- args[57]
A.caller.one <- args[58]
...
nonA.caller.two <- args[59]
A.caller.two <- args[60]
...
nonA.caller.three <- args[61]
A.caller.three <- args[62]
...
filtering_function <-
  function(subset_data,
           ...
           min.total.reads.eh, min.allele.reads.eh, min.vaf.eh, min.lc.eh,
  ...
  mutate(EH_passsample =
           ...
           case_when(sex == "M" & (chr == "chrX" | chr == "chrY") &
                    ...
                    E_VAF_1 >= min.vaf.eh & !is.na(E_VAF_1) &
                    # allele 2 VAF
                    E_VAF_2 >= min.vaf.eh & !is.na(E_VAF_2) &
                    # average locus coverage
                    E_LC >= min.lc.eh & !is.na(E_LC) ~ TRUE,
...
nonA_loci <-
  filtering_function(nonA_loci,
           ...
           nonA.min.total.reads.eh, nonA.min.allele.reads.eh, nonA.min.vaf.eh, nonA.min.lc.eh,
...
A_loci <-
  filtering_function(A_loci,
           ...
           A.min.total.reads.eh, A.min.allele.reads.eh, A.min.vaf.eh, A.min.lc.eh,
