...
# Usage: convert_to_EH.sh [Panel CSV] [list of EH exclusion loci] [full path to EH json output file]

# Panel CSV is a CSV that contains info directly from the shiny server for all loci in panel
# Panel CSV format: row.number,seqnames,start,end,width,period.size,motif,motif.family
# Panel CSV has 1-start, fully closed coordinates

# list of EH exclusion loci is a txt file containing one locus name per line
...
Rscript /scratch/cal8150/uSatAnalysis/scripts/eh_to_json.r ${OUT} $3
...
```{bash}
# Command line input
/scratch/cal8150/uSatAnalysis/scripts/convert_to_EH.sh /scratch/cal8150/uSatAnalysis/scaleup_usats_full.csv /scratch/cal8150/expansionhunter/eh_exclusion_loci.txt /scratch/cal8150/expansionhunter/scaleup_usats_eh.json
```
...
### e. Make Picard interval list (microsatellites)
...
### f. Make interval list (supplementary panel)

supp_bed_to_interval_list.sh
```{bash}
#!/bin/bash

# SBATCH --time=100

# Usage: supp_bed_to_interval_list.sh [supp_panel.bed] [output file full path] [reference genome dictionary file]
# output file must end in .interval_list

# supp_panel.bed is a bed that contains the coordinates for the targets of the supplementary capture panel
# supp_panel.bed has 0-start, half-open coordinates
# requires supp_panel.bed to be in the following order:
# [chr] [start] [end]
# Interval list coordinates are 0-start, half-open
...
sbatch /scratch/cal8150/uSatAnalysis/scripts/supp_bed_to_interval_list.sh /scratch/cal8150/uSatAnalysis/loci/supp_panel.bed /scratch/cal8150/uSatAnalysis/scripts/supp_bed_to_interval_list.sh /scratch/cal8150/uSatAnalysis/loci/supp_panel.interval_list /scratch/projects/evronylab/reference-files/hg38-gatk/Homo_sapiens_assembly38.dict
```
...
Sample TSV format: sampleID sex path/to/read1.fastq path/to/read2.fastq trio_name
Fields are TAB-SEPARATED.
The read 1 and read 2 fields can contain a comma-separated list of FASTQ files for the same sample.
The read 1 and read 2 lists of FASTQ files MUST be in the same order because these are paired reads.
If the sample is not part of a trio, the trio name can be set to a random value. Trio values are only used for downstream Mendelian concordance analysis if desired.

Below is a sample TSV that can be used with the pipeline:
```{bash}
...
// TSV with fields: sample,sex,read1,read2,trio
...
process ALIGN_BAM {
  ...
  # bwa mem -R [read group] -t 16 [reference fasta] [read 1 fastq] [read 2 fastq]
...
if (sex == "M") {
  ...
  results <- results %>%
    ...
    mutate(H_REPDIFF_1 = H_GB_1/motifLength,
           H_REPDIFF_2 = if_else((chr == "chrX" | chr == "chrY"), -999, H_GB_2/motifLength)) %>%
    # calculate repeat count
    # fill in -999 for allele 2 on sex chromosomes to indicate it should not be considered a true call
    mutate(H_REPCN_1 = (width + H_GB_1)/motifLength,
           H_REPCN_2 = if_else((chr == "chrX" | chr == "chrY"), -999, (width + H_GB_2)/motifLength))
  ...
  results <- results %>% 
    ...
    mutate(H_ALLREADS_1 = H_ALLREADS_1_TEMP, H_ALLREADS_2 = H_ALLREADS_2_TEMP,
           H_GB_1 = H_GB_1_TEMP, H_GB_2 = H_GB_2_TEMP) %>%
    mutate(H_REPDIFF_1 = if_else(chr == "chrY", -999, H_GB_1/motifLength), 
           H_REPDIFF_2 = if_else(chr == "chrY", -999, H_GB_2/motifLength)) %>%
    mutate(H_REPCN_1 = if_else(chr == "chrY", -999, (width + H_GB_1)/motifLength), 
           H_REPCN_2 = if_else(chr == "chrY", -999, (width + H_GB_2)/motifLength))
...
## 3. usat_genotype_filtering.r
Filtering script for use with YAML config file.
```{r}
#!/share/apps/r/4.2.0/gcc/bin/Rscript

# usage: Rscript usat_genotype_filtering.r [RDS output of join_str_samples_nextflow.r] [YAML config]
...
error_report_function <-
  ...
  # fill with "none" if passed
  # allele 2 filters are applied only to autosomes and female sex chromsomes

  subset_data <- subset_data %>%

    mutate(sample_failed_filters = "none") %>%
    mutate(sample_failed_filters = if_else(H_GT != "." & !is.na(H_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"no.gt.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_Q >= min.qual.hipstr & !is.na(H_Q),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.qual.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MALLREADS_SUM >= min.mallreads.hipstr & !is.na(H_MALLREADS_SUM),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.mallreads.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_ALLREADS_SUM >= min.allreads.hipstr & !is.na(H_ALLREADS_SUM),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.allreads.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MALLREADS_1 >= min.allele.mallreads.hipstr & !is.na(H_MALLREADS_1),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.mallreads.hipstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_MALLREADS_2 >= min.allele.mallreads.hipstr & !is.na(H_MALLREADS_2),
                                             sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.mallreads.hipstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_ALLREADS_1 >= min.allele.allreads.hipstr & !is.na(H_ALLREADS_1),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.allreads.hipstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_ALLREADS_2 >= min.allele.allreads.hipstr & !is.na(H_ALLREADS_2),
                                             sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.allreads.hipstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_MVAF_1 >= min.mvaf.hipstr & !is.na(H_MVAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.mvaf.hipstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_MVAF_2 >= min.mvaf.hipstr & !is.na(H_MVAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.mvaf.hipstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_AVAF_1 >= min.avaf.hipstr & !is.na(H_AVAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.avaf.hipstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             H_AVAF_2 >= min.avaf.hipstr & !is.na(H_AVAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.avaf.hipstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_STUTTERFRAC <= max.stutter.frac.hipstr & !is.na(H_STUTTERFRAC),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"max.stutter.frac.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_FLINDELFRAC <= max.flankindel.frac.hipstr & !is.na(H_FLINDELFRAC),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"max.flankindel.frac.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(H_GLDIFF >= min.gldiff.hipstr | is.na(H_GLDIFF),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.gldiff.hipstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_GT != "." & !is.na(G_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"no.gt.gangstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_Q >= min.qual.gangstr & !is.na(G_Q),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.qual.gangstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_ENCLREADS_SUM >= min.total.reads.gangstr & !is.na(G_ENCLREADS_SUM),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.total.reads.gangstr",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_ENCLREADS_1 >= min.allele.reads.gangstr & !is.na(G_ENCLREADS_1),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.reads.gangstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             G_ENCLREADS_2 >= min.allele.reads.gangstr & !is.na(G_ENCLREADS_2),
                                             sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.reads.gangstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(G_VAF_1 >= min.vaf.gangstr & !is.na(G_VAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.vaf.gangstr_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             G_VAF_2 >= min.vaf.gangstr & !is.na(G_VAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.vaf.gangstr_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_GT != "." & !is.na(E_GT),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"no.gt.eh",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(B_depth >= min.total.reads.eh & !is.na(B_depth),
                                           sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.total.reads.eh",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_ADSP_1 >= min.allele.reads.eh & !is.na(E_ADSP_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.allele.reads.eh_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             E_ADSP_2 >= min.allele.reads.eh & !is.na(E_ADSP_2),
                                             sample_failed_filters,
                                           str_remove(str_c(sample_failed_filters,"min.allele.reads.eh_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_VAF_1 >= min.vaf.eh & E_VAF_1 <= max.vaf.eh & !is.na(E_VAF_1),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.vaf.eh_1",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(((sex == "M" & chr != "chrX" & chr != "chrY") | (sex == "F" & chr != "chrY")) &
                                             E_VAF_2 >= min.vaf.eh & E_VAF_2 <= max.vaf.eh & !is.na(E_VAF_2),
                                             sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.vaf.eh_2",sep=";"),"none;"))) %>%
    mutate(sample_failed_filters = if_else(E_LC >= min.lc.eh & !is.na(E_LC),
                                           sample_failed_filters,str_remove(str_c(sample_failed_filters,"min.lc.eh",sep=";"),"none;"))) %>%
  ...
  # fill with "none" if passed
    ...
    mutate(locus_failed_filters = "none") %>%
    mutate(locus_failed_filters = if_else(mean_qual_H >= min.mean.qual.hipstr & !is.na(mean_qual_H),
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"min.mean.qual.hipstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_qual_G >= min.mean.qual.gangstr & !is.na(mean_qual_G),
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"min.mean.qual.gangstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_mdepth_H >= min.mean.mallreads.hipstr,
                                          locus_failed_filters,
                                          str_remove(str_c(locus_failed_filters,"min.mean.mallreads.hipstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_adepth_H >= min.mean.allreads.hipstr,
                                          locus_failed_filters,
                                          str_remove(str_c(locus_failed_filters,"min.mean.allreads.hipstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_depth_G >= min.mean.total.reads.gangstr,
                                          locus_failed_filters,
                                          str_remove(str_c(locus_failed_filters,"min.mean.total.reads.gangstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(mean_depth_E >= min.mean.total.reads.eh,
                                          locus_failed_filters,
                                          str_remove(str_c(locus_failed_filters,"min.mean.total.reads.eh",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_H == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"min.sample.frac.hipstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_G == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"min.sample.frac.gangstr",sep=";"),"none;"))) %>%
    mutate(locus_failed_filters = if_else(sample_frac_filter_E == T,
                                          locus_failed_filters,str_remove(str_c(locus_failed_filters,"min.sample.frac.eh",sep=";"),"none;")))
...
## 4. genotype_qc.r (for non-Mendelian data)

The same statistics will be output by trio_concordance.r, so this step can be skipped if you plan on running Mendelian trio analysis

```{r}
#!/share/apps/r/4.2.0/gcc/bin/Rscript

# usage: Rscript genotype_qc.r [filtered genotypes RDS]

# Outputs TSV with genotype quality data without accounting for Mendelian relationships
# genotypes RDS is output of output_genotypes.r with filtered genotype data
...
for (i in unique(results$sampleID)) {
# non poly-A loci stats

  nonA_results_stats <- results %>% filter(sampleID == i, motif.family != "A")
  A_results_stats <- results %>% filter(sampleID == i, motif.family == "A")
  ...
  nonA_stats <-
    data.frame(
      ...
      nonA_num_loci_genotyped = length(which(!is.na(nonA_results_stats$allele_1))),
      nonA_num_2bp_loci_genotyped = length(which(nonA_results_stats$motifLength == 2 & !is.na(nonA_results_stats$allele_1))),
      nonA_num_3bp_loci_genotyped = length(which(nonA_results_stats$motifLength == 3 & !is.na(nonA_results_stats$allele_1))),
      nonA_num_4bp_loci_genotyped = length(which(nonA_results_stats$motifLength == 4 & !is.na(nonA_results_stats$allele_1))),
      nonA_mean_spanning_depth = mean(nonA_results_stats$B_depth, na.rm = T),
      nonA_mean_hipstr_stutter = mean(nonA_results_stats$H_STUTTERFRAC, na.rm = T),
      nonA_mean_hipstr_quality = mean(nonA_results_stats$H_Q, na.rm = T),
      nonA_mean_gangstr_quality = mean(nonA_results_stats$G_Q, na.rm = T)) %>%
    ...
    mutate(
      ...
      nonA_frac_loci_genotyped_over_nonA_num_passing_filters = nonA_num_loci_genotyped / nonA_num_loci_passing_filters,
      nonA_frac_2bp_loci_genotyped_over_nonA_num_passing_filters = nonA_num_2bp_loci_genotyped / nonA_num_loci_passing_filters,
      nonA_frac_3bp_loci_genotyped_over_nonA_num_passing_filters = nonA_num_3bp_loci_genotyped / nonA_num_loci_passing_filters,
      nonA_frac_4bp_loci_genotyped_over_nonA_num_passing_filters = nonA_num_4bp_loci_genotyped / nonA_num_loci_passing_filters)
  ...
  A_stats <-
    data.frame(
      ...
      A_num_loci_genotyped = length(which(!is.na(A_results_stats$allele_1))),
      A_mean_spanning_depth = mean(A_results_stats$B_depth, na.rm = T),
      A_mean_hipstr_stutter = mean(A_results_stats$H_STUTTERFRAC, na.rm = T),
      A_mean_hipstr_quality = mean(A_results_stats$H_Q, na.rm = T),
      A_mean_gangstr_quality = mean(A_results_stats$G_Q, na.rm = T)) %>%
    ...
    mutate(
      ...
      A_frac_loci_genotyped_over_nonA_num_passing_filters = A_num_loci_genotyped / A_num_loci_passing_filters)

  statslist[[i]] <- cbind(nonA_stats, A_stats)

}  

all_loci_stats <- do.call("rbind",statslist)

# save genotyping QC statistics in TSV file
write.table(t(all_loci_stats), file = paste0(sub('\\.rds$', '', args[1]),"_genotype_qc_stats.tsv"), quote = F, row.names = T)
```
...
stutter_plot_func <- function(filtered_data, plot_title) {
  ...
  plotting_data <- filtered_data %>%
    ...
    mutate(unit_diff = bp_diff_from_call / motifLength)
...
condition_1_2bp <- condition_1_all %>% filter(sex == "M", chr == "chrX", motifLength == 2, H_GT != ".")
condition_1_3bp <- condition_1_all %>% filter(sex == "M", chr == "chrX", motifLength == 3, H_GT != ".")
condition_1_4bp <- condition_1_all %>% filter(sex == "M", chr == "chrX", motifLength == 4, H_GT != ".")
